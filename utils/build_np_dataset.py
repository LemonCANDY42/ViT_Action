...
def process_all_videos(video_folder, output_root, fps: int = 1, platform="cuda", use_last_state=True):
	...
	video_files = [video for video in video_folder.glob('**/*') if
				   video.suffix.lower() in VIDEO_EXTENSIONS and str(video) not in processed_videos]
	...
	try:
		...
		with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
			futures = {executor.submit(extract_frames, video_file, output_root, fps, platform): video_file for
					   video_file in
